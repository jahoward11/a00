...
// reShow(Array.from(Array(64).keys()).map(n => (n += 0x2000) && [0, 64, 128, 192].map(d => "0x" + (n + d).toString(16) + " " + String.fromCodePoint(n + d)).join("  \\t")).join("\\n"))
...
// window.ecoqjs || scrInj("../-app-eco/eco-srvc1.js").catch(reShow)
// Object.fromEntries( Object.entries(ecoqjs).map(oe => [oe[0], oe[1].toString()]) )
// fetch("../-app-eco/eco-srvc1.js").then(re => re.text()).then(ecoqjs.jcmtXtract)
...
// caches.open("calcjs0.14").then(cache => cache.delete("https:/" + "/jahoward11.github.io/a00/-res-js/jstat-tdist.js")).then(reShow)
...
// localforage.getItem("tutor2js").then(reShow).catch(reShow)
