...
const xchar2htmlnc = (str, idx, feats = [{}]) => {
  ...
  (feats[0].htmlxncodes || htmlxncodes)
  .forEach(nc => str = str.replace(RegExp("\\" + nc[0], "g"), nc[1]));
  return str;
};

const xchar2htmlnc1 = (str, idx, feats = [{}]) => {
  // convert x-character glyph to respective html name code
...
const htmlnc2xchar = (str, idx, feats = [{}]) => {
...
const htmlnc2xchar1 = (str, idx, feats = [{}]) => {
  // convert html name code to respective x-character glyph
  // immunize html x, pre, code element from conversion
...
const srcCodeIncl = (str, idx, feats) => {
...
const menubarGen = str => {
  ...
  .replace( /(<div class=['"]?menubar['"]?>)([^]*?)(?=<\/div>)/, (m, c1, c2) =>
    ...
    .replace(/<li><p>([^\n<].+?)(?=<\/a>|<span><\/span><\/a>)/g, (m2, c21) =>
      '<li id=mbar' + (++mbarid) + '-' + hdgIDGen(c21) + '><a href="">' + c21) + "\n</div>" );
};
...
export {
  ...
  latchars, mditPrepPhase2, mditPrepPhase5,
  mdtblConv21, menubarGen, moduleIX,
  spachars, symchars, srcCodeIncl,
  xchar2htmlnc, xchar2htmlnc1
};
