  ...
  let dbpc2, dbteam, apath, rval, sdir,
    ...
    listsRfrsh = msgerr => {
      ...
      tm0urole = dbpch && ( tm0cntcs[epsets.uname]
        ...
        || (caccts.find(e => e.DBNAME === dbpch.name) || "").USRNAM );
...
function rmtListGen() {
  ...
  const context = {
    rmtitems: caccts.map( e => Object.assign(
      { seltxt: (e.DBNAME || "").replace(/^a\d\d_(?=\w)/, "@") + (!e.DBPUBL ? "" : " †") }, e ))
  };
...
function fwResets(invalid) {
  let eftypes = document.querySelectorAll('#ecoesp0 #tooltypes>.eftype'),
    metas = document.querySelectorAll('#ecoesp0 #sdirinp, #ecoesp0 #jfidinp, #ecoesp0 #versinp, #ecoesp0 #ownrinp, #ecoesp0 #peoptxta, #ecoesp0 #misctxta, #ecoesp0 #delswi');
  lnkstor = [];
...
// data sources: 1 valcon-filepath/json->txdata, 2 pchSel/ibmConnect-obj->txdata
  ...
  } else if (txdata.CLEANUP && (dbpch && dbpch.name === txdata.DBNAME || txdata.DBNAME && dburl)) {
    (!dburl ? dbpch : new PouchDB(dburl, { skip_setup: true }))
...
function fwUpdPrep(fileref, dirref, pchutrg, lfnew) {
  ...
  // replaces filewkg with detele obj only when: delswi is checked
  ...
  || /^~[a-z]{3}[AP]|^~[amp]\d{8}/.test(jfw._id))
  && ucallsgn && ( pchutrg || lfnew || !/^~[a-z]{3}[AP]\d{8}|^~[amp]\d{8}/.test(jfw._id)
  || /^(?:Anno|Memo|Post)\d{8}/.test(fileref) && jfw._id.replace(/^~[a-z]{3}[AP]|^~[amp]/, "")
  ...
  && !/^~DVIZ_|^~TMP[\dL]_|^![A-Za-z]{3}/.test(jfw._id) ) {
    jfw._id = fileref;
    jfw._rev = "";
    jfw.ts_created = tstamp1;
  } else if ( jfw.file_created && jfw._id !== fileref
  && !/^~DVIZ_|^~DBID_|^~TMP[\dL]_|^~[a-z]{3}[AP]/.test(jfw._id) ) {
    jfw._id = fileref;
    jfw._rev = "";
    ptys0Add("file_created");
  }
  if (jfw.hasOwnProperty("ts_updated") && !/^~TMP[\dL]_/.test(jfw._id)) {
    ...
    jfw.file_type !== "eco-publmgr" || !pchutrg
    || (jfw.file_updated.version = jfw.file_updated.version.replace(/\d+$/, m => ++m))
    && jfw.contributors.some(e => e === epsets.uname) || jfw.contributors.push(epsets.uname);
  }
  ...
  && !/^~DVIZ_|^~DBID_|^~TMP[\dL]_|~[a-z]{3}[AP]/.test(jfw._id) ) {
    ...
    || !pf3stor.dbsdir.some(e => e[1] && e[1].replace(/^\./, "") === dirref) )) {
      ...
      .replace(/("_id":")7-assets",/, "$1" + dirdot + dirref + "\",\"_rev\":\"\",")
  ...
  if (jfw.file_type === "eco-publmgr") {
    !versinp.classList.contains('is-warning') || (jfw.file_updated.version = versinp.value.trim());
...
ibmConnect() {
  let dbpc1, dbpc2, pcloud, ridx, rmtdn, tm2txd, userinfo,
    ...
    //tm0ipch = valinp[2] // todo: possibly identify 2ndary team dbs
      ...
      //&& caccts.some(e => e.DBNAME === "a00_" + valinp[2] && e.DBORIG && e.USRNAM),
    ...
    teamDSet = () => {
      ...
      .then((rsltk = {}) => {
        if (rsltk.dbs) {
          ...
          return !dbpc1 || !(tm2txd = caccts.find(e => e.DBNAME === dbpc1.name)) || !tm2txd.USRNAM
          ? espEnter()
          : dbpc1.info().then(resp => {
            dbpc2 = new PouchDB(txurlGen(tm2txd), { skip_setup: true });  // slow sync team db
              ...
              .then( () => tm0urole = tm0cntcs[epsets.uname]
                && tm0cntcs[epsets.uname].roles.find(e => /^Admin$|^Lead$|^Contributor$/i.test(e))
                || (caccts.find(e => e.DBNAME === valinp[1] || fbktxd.DBNAME) || "").USRNAM )
