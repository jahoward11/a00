...
clRanks = () => { _.cras = _.tcx > 1 && _.unsh.map( v => _.ovr < 0 ? _.clrefs[_.tcx][_.cxs.find(x => x === 6 || v <= _.cval || (v % _.cval > 0 && v % _.cval <= _.cval - _.rval + 1) || (v > _.cval * (1 + x) - _.rval + (1 + x) && v <= _.cval * (1 + x)) || (v - (1 + x) - (_.cval - _.rval)) % _.cval === 0)] : _.clrefs[_.tcx][_.cxs.find(x => x === 6 || v - _.ovr <= _.max * (1 + x) || (v - (1 + x)) % _.max === 0)] ); _.cra2 = _.tcx > 1 && _.unsh.map( v => _.ovr < 0 ? (v <= _.cval - _.rval || (v - 1 - (_.cval - _.rval)) % (1 + _.cval) !== 0 || v > 7 * _.cval ? 'White' : _.tcx < 5 ? 'Gold' : 'LimeGreen') : v - 1 < _.ovr || (v - 1 - _.ovr) % (1 + _.max) !== 0 || v - 1 - _.ovr > 7 * _.max ? 'White' : _.tcx < 5 ? 'Gold' : 'LimeGreen' ); }; ""
