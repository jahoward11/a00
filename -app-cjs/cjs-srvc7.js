...
__Puzzle Collection I: from JS Tutorial 2__
 ...
 This module screen functions only to display all of the puzzle GUIs
that are already built within the \`JScode/tutorial2\` data file:
- SLIDING TILES
- LIGHTS OUT
- PEGS
*/

ptm = xstor["JScode"]["tutorial2"]; ""
bodGen = rslt => "<h3 class=cfield>Puzzles, JS Tutorial 2</h3>\\n\\n" + rslt.match(/^g\\dui = [^]+?(?=\\n$)/gm).map(e => e.replace(/\\bg\\dwrap\\b/g, "pz1wrap").replace(/; ""$|^g\\dui = /g, "").split(/; ""\\ng\\dui \\+= /).map(eval).join("").trim()).join("\\n\\n"); "" //
scrGen = rslt => rslt.match(/^(?:jopts|m2trk|tnx) = [^]+?(?=\\n+ *\\/[\\/*])/gm).map(e => "let " + e.replace(/\\b_\\.\\b| *"";?$|^\\n/gm, "").replace(/^.+/, m => m.replace(/ *=(?= *[a-z]|$)/gi, ",")).replace(/^(\\w+ =.+);(?=\\n\\w+ =)/gm, "$1,")).join("\\n"); "" //
...
try { pz1wrap } catch { uiDspl(bodGen(ptm)); scrInj(null, 'module', scrGen(ptm)).catch(respShow); }
...
// respShow((dwrap[0] + bodGen(ptm) + dwrap[1] + scrGen(ptm) + dwrap[2]).replace(/<(?=[!/?a-z])/gi, "&lt;"))
//`;
...
g4view = { canvas: document.createElement("canvas"), start: function () { this.canvas.width = this.canvas.height = 308; this.context = this.canvas.getContext("2d"); g4board.appendChild(this.canvas); this.interval = setInterval(_.vRfrsh, 4); }, clear: function (pc) { !pc ? this.context.clearRect(0, 0, this.canvas.width, this.canvas.height) : Array.isArray(pc) ? this.context.clearRect(...pc) : this.context.clearRect(pc.x, pc.y, pc.wi, pc.ht); }, stop: function () { clearInterval(this.interval); } };
compnX = function (clr, x, y, w, h, r = 0) { this.wi = w || 2 * r; this.ht = h || 2 * r; this.x = x; this.y = y; this.xsp = 0; this.ysp = 0; this.pcDrw = () => { let ctx = _.g4view.context; ctx.fillStyle = clr; if (!r) { ctx.fillRect(this.x, this.y, this.wi, this.ht); } else { ctx.beginPath(); ctx.arc(this.x + r, this.y + r, r, 0, 2 * Math.PI); ctx.fill(); } }; this.posChg = () => { this.x += this.xsp; this.y += this.ysp; }; this.posRvrs = d => d == 1 ? (this.y += 1) : d == 2 ? (this.y -= 1) : d == 3 ? (this.x += 1) : d == 4 ? (this.x -= 1) : 0; this.collDt = () => { let [bt, bb, bl, br] = [this.y, this.y + (this.ht), this.x, this.x + (this.wi)]; _.t0trk || (_.t0trk = 1 + _.tstrk.findIndex(([tx, ty]) => bt < ty && bb > ty && bl < tx && br > tx)); return ( !(bt > 1 && bb < _.g4view.canvas.height - 1 && bl > 1 && br < _.g4view.canvas.width - 1) || _.g4sets[_.midx].some( ([ox, oy, ow, oh]) => !(bb < oy - 1 || bt > oy + oh + 1 || br < ox - 1 || bl > ox + ow + 1) )); }; }; ""
vRfrsh = () => { _.g4view.clear(_.gball); _.gball.posChg(); if (!_.gball.collDt() && !_.t0trk) { _.dbar = 0; _.gball.pcDrw(); } else if (!_.t0trk) { _.dbar = _.dbar || (_.gball.ysp < 0 ? 1 : _.gball.ysp > 0 ? 2 : _.gball.xsp < 0 ? 3 : _.gball.xsp > 0 ? 4 : 0); _.gball.xsp = _.gball.ysp = 0; _.gball.posRvrs(_.dbar); } else { !_.tstrk.length || _.g4view.clear( (_.tstrk.splice(_.t0trk - 1, 1)[0] || []) .map(e => e - (_.midx == 6 ? 14 : 15)).concat(_.midx == 6 ? [28, 29] : [30, 30]) ) || (_.t0trk = 0) || ( g4movs.innerHTML = "<em>" + ++_.t1trk + " targets achieved!" + (_.tstrk.length ? "</em>" : " Maze completed!</em>") ); } }; ""
...
bodGen = rslt => rslt.match(/^g\\dui = [^]+?(?=\\n$)/m)[0].replace(/; ""$|^g\\dui = /g, "").split(/; ""\\ng\\dui \\+= /).map(eval).join("").trim(); "" //
scrGen = rslt => "let " + rslt.match(/^dbar = [^]+?(?=\\n+ *\\/[\\/*])/m)[0].replace(/\\b_\\.\\b| *"";?$|^\\n/gm, "").replace(/^.+/, m => m.replace(/ *=(?= *[a-z]|$)/gi, ",")).replace(/^(\\w+ =.+);(?=\\n\\w+ =)/gm, "$1,"); "" //
