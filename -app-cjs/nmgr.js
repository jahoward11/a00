...
const nmscr = `let cvs, fwg, rva2, rval, ss0, ss1, vbas, vusr,
  ...
  fileLoad = evt => {
    let lang,
      ...
      attVw = abl => reximg.test(txd2.ATTKEY) || abl && /^image/.test(abl.type)
        ...
        : fetch(aurls[txd2.ATTKEY] || abl && (aurls[txd2.ATTKEY] = URL.createObjectURL(abl)))
          .then(r => r.text()).then(str => nmdata.innerHTML = jp1(str)).catch(rsp2Show);
    //rsp2Show(txd2);
    ...
    : !finp.checked ? tabActv() || PouchDB(txd2.DBNAME).get(txd2.FILEID, txd2.OPTS)
        .then( doc => nmdata.innerHTML = jp1( !finp &&
          ( doc.hasOwnProperty("content") ? doc.content : !Array.isArray(doc.filefrags) ? 0
            : doc.filefrags.map(ff => ff.contenttxt).filter(e => e).join("\\n\\n") )
  ...
  filtExe = () => {
    ...
    document.querySelectorAll('#nmclip .media').forEach(e => e.classList.remove("dnone"));
  ...
  tabActv = evt => {
    ...
    [nmflow, nmclip].some(e => !e.classList.contains('dnone')) || (viewsel.selectedIndex = vidx = tidx);
  ...
  viewRte = evt => {
    [nmflow, nmclip].forEach(e => e.classList.add("dnone"));
    ...
    if (!window.PouchDB || !txd1.DBNAME) {
      ...
      || [nmflow, nmclip].some(e => !e.classList.contains('dnone')) || tabActv();
    } else if (vbas < 2) {
  ...
  viewChg = evt => {
    ...
    if (vidx < 4 && evt && [nmflow, nmclip].every(e => e.classList.contains("dnone"))) {
      return tabActv(evt);
    }
