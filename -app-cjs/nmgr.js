...
#nmwrap .textarea { font: normal medium/normal monospace; }
    ...
    if (tidx === 1 && !fwg && txd3.DBNAME) {
      ...
      !fwg.file_created || [[fwg.file_created, fwg.file_updated]].forEach(([p0, p1]) => {
        ...
        p0.dborigin || ( p1.dborigin = p0.dborigin
          = /127\\.0\\.0|192\\.168\\.0|cloudant|localhost/.test(location.origin)
          ? location.origin : nm0cfgs[txd3.DBNAME] && nm0cfgs[txd3.DBNAME].remot.orig || "" );
    ...
    <div class="field-body">
      ...
      sav2btn.onclick = () => {
        ...
        .put(Object.assign({ _id: "", _rev: "" }, fwg), {}).then( trsp =>
          Object.keys(fwg).forEach( k => /^_rev$/.test(k)
      ...
      Object.keys(fwg).forEach(k => /^_rev$/.test(k) || (window["p0" + k].onblur = dataVld));
    }
  },
  viewRte = evt => {
