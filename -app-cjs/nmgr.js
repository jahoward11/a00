...
#nmwrap .table {
  line-height: normal;
  min-width: calc(219px + 35vw);
  margin-left: 0.5rem;
} /* 219/375px=58vw, 350/375px=93vw */
...
#nmbulk #qselbtn { margin-right: 7px; }
...
const nmscr = `let cvs, fwg, rva2, rval, ss0, ss1, vbas, vusr,
  ...
  qslrs = [
    ...
    + '#nmdata .media-content>details:first-of-type>summary>strong, '
    + '#nmdata .media-content>details:first-of-type>summary>em, '
    + '#nmdata .media-content>details:first-of-type>div, '
    + '#nmdata .media-content>details:last-of-type>summary, '
    + '#nmdata .media-content>details:last-of-type>div>div',
    '#nmtbl thead:not(.dnone) td:first-of-type>input.dnone',
    '#nmdata .media-content>details:first-of-type>summary>span.dnone',
    '#nmtbl tbody>tr:not([id])>td:first-of-type>input:checked',
    '#nmdata .media-content>details:first-of-type>summary input:checked',
    '#nmtbl tbody>tr:not([id])>td:first-of-type>input',
    '#nmdata .media-content>details:first-of-type>summary input',
    '#nmdata .media-left>.image>img, #nmdata .media-content>details:first-of-type a'
  ],
  atthtm = \`
  <hr />
  <div class=cfield>
  ...
  storDel = () => {
    ...
    !spdbswi.checked || (txd1.DBNAME = "") || !window.PouchDB || Promise.all( dbs.map( dbi =>
      ...
      .then(trsp => rsp2Show("Local DB successfully destroyed: " + dbi))
      .catch(err => {
  ...
  <div id=qchlp class="chelp iwarn dnone"><strong>ALERT:</strong> Invalid JSON.</div>\\n\`;
    ...
    if (vbas > 1) {
      ...
      .forEach(elm => elm.classList.remove("hsuccl", "hwarnl"));
  ...
  fileLoad = evt => {
    let sdir = vbas > 1 && evt && evt.target.parentElement.previousElementSibling.textContent,
      ...
      attVw = abl => reximg.test(txd2.ATTKEY) || abl && /^image/.test(abl.type)
        ...
        : fetch(aurls[txd2.ATTKEY] || abl && (aurls[txd2.ATTKEY] = URL.createObjectURL(abl)))
          .then(r => r.text()).then(str => nmbulk.innerHTML = jp1(str)).catch(rsp2Show);
    //rsp2Show(txd2);
    ...
    : /^~[a-z]/.test(txd2.FILEID)
    ? tabActv() || PouchDB(txd2.DBNAME).get(txd2.FILEID, txd2.OPTS)
        .then( doc => nmbulk.innerHTML
          = "\\n<pre class=pwrap>" + valStr(Object.assign({ _id: "", _rev: "" }, doc), 2) + "</pre>\\n" )
        .catch(rsp2Show)
    ...
    <div class="field-body">
      <textarea id=p0\${k} class=textarea rows=\${taHt(k)}>\${htmTxt(str)}</textarea>
        ...
        updbtn.onclick = () => {
          ...
          !chgs || !txd2.DBNAME || !window.PouchDB || PouchDB(txd2.DBNAME)
  ...
  bulkTog = () => {
    ...
    document.querySelectorAll( vbas > 1 ? qslrs[4]
      : '#nmdata .media-content>details:first-of-type>summary>span' )
  ...
  tblGen = evt => {
    ...
    .then(re => {
      ...
      nmtbod.innerHTML = qd5Fmt( ss0 === "seq" ? dels
    ...
    .then(re => {
      ...
      ftotal.innerText = rdc ? rdc : vbas !== 2 ? re.rows.length //re.total_rows
        : ( r2s = re.rows.filter( r => (rval = r.doc || r.value).hasOwnProperty("loc_subdir")
            || rval.file_updated || rval.file_created )).length
          + calcSum( r2s.filter(r => (r.doc || r.value)._attachments)
  ...
  <article class=media>
    ...
    <div class="media-content">
      <details class=cfield>
        <summary>
          ...
          <em class="diblk fsz0c75">\${ !cvs.some(e => /timestamp/.test(e))
            ...
            ? "" : new Date( ra.ts_updated || ra.ts_created
              || (ra.file_updated || ra.file_created || "").timestamp || null ).toLocaleString() }</em>
        </summary>
        <div class="pwrap fsz0c75">\${
          ...
          + ( !cvs.includes("file_created.timestamp") || /anno|memo|post/i.test(ra.file_type)
            ...
            + new Date(ra.ts_created || (ra.file_created || "").timestamp || null).toLocaleString()
            + "\\n" )
          + ( !cvs.includes("file_updated.timestamp") || /anno|memo|post/i.test(ra.file_type)
              || !ra.ts_updated && !ra.file_updated ? "" : "UPDATED: "
            + new Date(ra.ts_updated || ra.file_updated.timestamp || null).toLocaleString() ) }</div>
      </details>
      ...
      hdrsswi.onchange = ev2 =>
        document.querySelectorAll('#nmdata .media-content>details:first-of-type')
        .forEach(e => e.open = ev2.target.checked);
      bodsswi.onchange = ev2 =>
        document.querySelectorAll('#nmdata .media-content>details:last-of-type')
        .forEach(e => e.open = ev2.target.checked);
      document.querySelectorAll(qslrs[12]).forEach(e => e.onclick = fileLoad);
  ...
  tabActv = evt => {
    ...
    if (tidx === 1 && !fwg) {
      ...
      !fwg.file_created || [[fwg.file_created, fwg.file_updated]].forEach(([p0, p1]) => {
        ...
        p1.dborigin = p0.dborigin = /127\\.0\\.0|192\\.168\\.0|cloudant|localhost/.test(location.origin)
          ? location.origin : [navigator.userAgent || navigator.userAgentData, location.origin];
      ...
      savbtn.onclick = () => {
        ...
        || a1inp.files.forEach( (f, i) =>
          fwg._attachments[ !imganm2.value ? f.name : !a1inp.files[1]
            ? imganm2.value : imganm2.value.replace(/(?=\\.\\w+$|$)/, i > 9 ? i : "0" + i) ]
          = { content_type: f.type, data: f } );
        !chgs || !p0_id.value || !txd1.DBNAME || !window.PouchDB || PouchDB(txd1.DBNAME)
  ...
  cfgsUpd = evt => {
    ...
    && txd1.DBNAME && ddelinp.value === txd1.DBNAME && window.PouchDB) {
      ...
      evt.target.id !== "dbd0btn" || PouchDB(txd1.DBNAME).compact().then( trsp =>
          hlps[2].classList.remove("dnone") || rsp2Show(trsp) )
          //|| !vidx || (viewsel.selectedIndex = vidx = 0) || viewChg() )
      ...
      evt.target.id !== "dbd1btn" || PouchDB(txd1.DBNAME).destroy().then(trsp => {
        ...
        //cfg.remot.pw = ""; localStorage._nm0cfgs = JSON.stringify(nm0cfgs);
        hlps[2].classList.remove("dnone")
        || (pdbssel.selectedIndex = 0) || pdbOpen(0) || rsp2Show(trsp);
      }).catch(err => hlps[3].classList.remove("dnone") || rsp2Show(err));
  ...
  pdbSync = evt => {
    ...
    if (cfg && evt && evt.target.id === "pfilbtn") {
      ...
      : new PouchDB(txd1.DBNAME).sync(db2, { live: false })
        .then(trsp => {
          ...
          ? hlps[5].classList.remove("dnone") || rsp2Show(trsp)
          : hlps[4].classList.remove("dnone")
            || (nmbulk.innerHTML = "\\n<pre class=pwrap>" + valStr(trsp, 2) + "</pre>\\n");
        }).catch(err => hlps[5].classList.remove("dnone") || rsp2Show(err));
    }
  },
  pdbOpen = evt => {
    let cfg,
    ...
    pw1swi.disabled = !cfg;
    pdbupd.innerText = !txd1.DBNAME ? "" : new Date(cfg.tsupd || null).toLocaleString();
    pdbset.innerText = !txd1.DBNAME ? "" : new Date(cfg.tsset || null).toLocaleString();
  ...
  pdbsGen = (dbs = []) => {
    ...
    nm0sets && [ "uemail", "uname", "dbcntc", "dbdflt", "vwdflt", "bedit", "usecfg", "useset",
    ...
    && ["type0", "view0", "tsset", "tsupd", "tsdel", "user0", "descr", "remot"]
