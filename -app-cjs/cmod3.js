...
// __GIVEN VALUE (*key~0~*)__
 rate0 = 0.0525 / 12	//yearly interest rate ÷ 12 ^mos^/~yr~
 ppmt0 = 0
 ...
 dbeg0 = new Date("2022-07-01")

// __CALCULATED VALUE (*key~1~*)__
 ppmt1 = ppmt0 || rate0 * pval0 / [1 - Math.pow(1 + rate0, -nper0)]
 $ppmt1 = !nper0 ? "" : (Math.round(ppmt1 * 100) / 100).toFixed(2)

 pval1 = pval0 || [$ppmt1 / rate0] * [1 - Math.pow(1 + rate0, -nper0)]
 $pval1 = !nper0 ? "" : Math.round(pval1).toFixed(2)

 fval1 = fval0 || $pval1 * Math.pow(1 + rate0, nper0) - ($ppmt1 / rate0) * [Math.pow(1 + rate0, nper0) - 1]
 fint1 = $ppmt1 * nper0 - ($pval1 - fval1)
 $fint1 = !nper0 ? "" : (Math.round(fint1 * 100) / 100).toFixed(2)

 nper1 = -Math.log(1 - rate0 * $pval1 / $ppmt1) / Math.log(1 + rate0)
 $nper1 = nper0 || Math.round(nper1)
 nyrs1 = Math.round($nper1 * 10 / 12) / 10
 dend1 = new Date(new Date(dbeg0).getTime() + [$nper1 + 1] * 30.35 * 24 * 60 * 60 * 1000)
